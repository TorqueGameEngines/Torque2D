...
- Rotating a `ParticlePlayer` now actually steers its particles. An emitter whose `LinkEmissionRotation` is set -- which is the default -- adds the player's own angle to its emission angle, but the two were in different units: an emission angle is authored in degrees, while a scene object's angle is radians. The radians were added raw, so the rotation arrived at about a fifty-seventh of its size and the field looked like it did nothing at all; turning a player through 90 degrees swung its emission by 1.6. Attaching an effect to a moving object and rotating it so the exhaust trails behind now works. Content can only have relied on the old behavior by accident, but two toys change: `PointForceControllerToy`'s impact explosions now blow outward from the planetoid that was hit, which is what the toy's own angle calculation always asked for, and an asteroid's fire trail now turns with the asteroid it is jointed to.
- On Windows, a directory scan reported the spelling the string table already held rather than the one on disk. The string table's hash folds case and an ordinary insert returns whichever spelling of a name reached it first, so a folder genuinely called `sprites` came back from `FindFirstFile` as `Sprites` -- `SpriteBatch` interns that spelling as a taml node name during static initialisation, before any of the engine runs, and `guiProfileTheme` does the same for `Fonts`. Opening the folder still worked, because the filesystem does not care, but every caller that carries the name rather than the handle got a name that will not resolve anywhere case sensitive, which is how a copied `module.taml` came to say `Path="Sprites"`. The five enumeration sites in the Win32 back-end now intern case sensitively, as the Linux back-end already did. macOS is unchanged.
